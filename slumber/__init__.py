...
class Resource(ResourceAttributesMixin, object):
    ...
    def _request(self, method, data=None, params=None):
        ...
        params_new = dict(self._store['default_params'])
        params_new.update(params)
        resp = self._store["session"].request(method, url, data=data, params=params_new, headers={"content-type": s.get_content_type()})
